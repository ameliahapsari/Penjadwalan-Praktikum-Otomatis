...
private $PRAKTIKUM = 'PRAKTIKUM';
			...
			$start = $time;			
				...
				if($rs_data->num_rows() == 0){
					...
					// $total_menit = round(($total_time / 60), 4) ;
						$data['waktu'] = "Selesai dalam ".$total_time." detik";
					...
					$total_time = round(($finish - $start), 2) ;
					// $total_menit = round(($total_time / 60), 4) ;
					$data['waktu'] = "Berhasil! Proses generate selesai dalam ".$total_time." detik";	
					}
    ...
    {		
		...
		if($prodi==true){
			$rs_data = $this->db->query("SELECT   a.kode,"
									...
									. "LEFT JOIN asisten f "
                                    . "ON a.kode_asisten = f.kode "
									. "LEFT JOIN status_asisten g "
                                    . "ON f.status_asisten = g.kode "
        ...
        foreach ($rs_data->result() as $data) {
            ...
            $this->status_asisten[$i]       = intval($data->status_asisten);
        ...
        $rs_Waktuasisten = $this->db->query("SELECT a.kode_asisten,".
		...
		if($prodi==true){
						$rs_Waktutersedia = $this->db->query("SELECT a.kode,".		
											  ...
											  "b.kode,".
											  "b.kode_asisten,".
											  "CONCAT_WS(':',a.kode_hari,d.sesi,a.kode_ruang,b.kode_asisten) as kode_hari_ruang , ".
											  "c.kode ,".
											  "c.semester_tipe ".
		...
		if($prodi==true){
						...
						$rs_Waktutersimpan = $this->db->query("SELECT a.kode,".		
											  ...
											  "b.kode,".
											  "b.kode_asisten,".
											  "CONCAT_WS(':',a.kode_hari,d.sesi,a.kode_ruang,b.semester,b.kelas,b.kode_asisten,b.kode_prodi) as kode_hari_ruang , ".
											  "c.kode ,".
											  "c.semester_tipe ".
				...
				foreach ($rs_Waktutersimpan->result() as $data) {
					$this->itersimpan[$i]         = intval($data->kode_asisten);
					$this->itersimpann[$i]         = $data->kode_asisten;
					$this->waktu_tersimpan[$i][0] = intval($data->kode_asisten);
	...
	function excel_report(){
		// $data['penjadwalan'] = $this->M_Penjadwalan3->get(jadwalpelajaran)->result();
		...
		// $object->getProperties()->setCreator("Laboratorium Teknik Informatika");
		// $object->getProperties()->setLastModifiedBy("Laboratorium Teknik Informatika");
		// $object->getProperties()->setTitle("Jadwal Praktikum");
  ...
  //           return false;
		...
		//$fields =  array("hari", "sesi", "jam_kuliah", "nama_mk", "asisten", 
						//"jumlah_jam", "nama_kelas", "nama_semester", "nama_prodi", "kuota", "nama_jurusan", "ruang", "kapasitas");
		$fields =  array("hari", "ruang", "jam_kuliah", "nama_mk", "asisten", 
