...
class Penjadwalan2 extends CI_Controller {
...
private $PRAKTIKUM = 'PRAKTIKUM';
	...
	function __construct(){
		...
		$this->load->model("M_Penjadwalan2");
	...
	if($this->session->userdata('logged_in')==false) redirect('admin/index');		
		...
		if(!empty($_POST)){
			...
			$start = $time;			
				...
				$rs_data = $this->db->query("SELECT   a.kode,"
                                    ...
                                    . "WHERE b.semester%2 = $jenis_semester "
                                    . "      AND a.tahun_akademik = '$tahun_akademik' and d.kode_jurusan='$jurusan'");
				...
				if($rs_data->num_rows() == 0){
					...
					$this->AmbilData($jenis_semester, $tahun_akademik, $jumlah_populasi,$jurusan);
					...
					for($i = 0;$i < $jumlah_generasi;$i++ ){
						$fitness = $this->HitungFitness($jumlah_populasi,$jurusan);
						...
						$fitnessAfterMutation = $this->Mutasi($jumlah_populasi,$mutasi,$jurusan);
		...
		$data['rs_tahun'] = $this->M_Tahun->semua_tahun();
			...
			$data['rs_jadwal'] = $this->M_Penjadwalan2->get();
		$datas['aside']='penjadwalan_bar';
		$this->load->view('head',$datas);   
        $this->load->view('penjadwalan2',$data);   
        $this->load->view('footer');   		
	}
	
	    public function AmbilData($jenis_semester, $tahun_akademik, $jumlah_populasi,$jurusan)
    {		
			
			$this->jenis_semester = $jenis_semester;
			$this->tahun_akademik = $tahun_akademik;
			$this->populasi       = intval($jumlah_populasi);
		...
		if($jurusan==true){
           $rs_data = $this->db->query("SELECT   a.kode,"
                                    ...
                                    . " AND a.tahun_akademik = '$this->tahun_akademik' and d.kode_jurusan ='$jurusan' ");
        }
		else{
		   $rs_data = $this->db->query("SELECT   a.kode,"
                                    . "       b.jumlah_jam,"
        ...
        foreach ($rs_data->result() as $data) {
			...
			$this->jurusan[$i]       = intval($data->kode_jur);
        ...
        $rs_RuangReguler = $this->db->query("SELECT kode 
                                            FROM ruang 
                                            WHERE jenis = '$this->TEORI' and kode_jurusan='$jurusan'");
		}
        ...
        $rs_Ruanglaboratorium = $this->db->query("SELECT kode "
                                                 ."FROM ruang "
                                                 ."WHERE jenis = '$this->LABORATORIUM' and kode_jurusan='$jurusan'");
		}
		else{
		$rs_Ruanglaboratorium = $this->db->query("SELECT kode "
                                                 ."FROM ruang "
		...
		if($jurusan==true){
						$rs_Waktutersedia = $this->db->query("SELECT a.kode,".		
											  ...
											  "b.kode,".
											  "b.kode_asisten,".
											  "CONCAT_WS(':',a.kode_hari,a.kode_jam) as kode_hari_ruang , ".
											  "c.kode ,".
											  "c.semester_tipe ,".
											  "d.kode ,".
											  "d.nama_prodi ".
											  ...
											  " AND b.tahun_akademik = '$this->tahun_akademik' AND d.kode_jurusan != '$jurusan'");
				$i             = 0;
    ...
    public function Inisialisai($jumlah_populasi)
    {
        ...
        for ($i = 0; $i < $this->populasi; $i++) {
            ...
            for ($j = 0; $j < $jumlah_pengampu; $j++) {
                ...
                } else if ($this->jenis_mk[$j] === $this->TEORI) {
    ...
    private function CekFitness($indv,$jurusan)
    {
        ...
        for ($i = 0; $i < $jumlah_pengampu; $i++)
        {
		  ...
		  $jurusan_a = intval($this->jurusan[$i]);        
            ...
            for ($j = 0; $j < $jumlah_pengampu; $j++) {                 
                ...
                $jurusan_b = intval($this->jurusan[$j]);
    ...
    public function HitungFitness($jumlah_populasi,$jurusan)
    {
        ...
        {            
            $fitness[$indv] = $this->CekFitness($indv,$jurusan);            
        }
    ...
    public function Mutasi($jumlah_populasi,$mutasi,$jurusan)
    {
        $this->populasi       = intval($jumlah_populasi);
        $this->mutasi         = $mutasi;
        ...
        for ($i = 0; $i < $this->populasi; $i++) {
            ...
            $fitness[$i] = $this->CekFitness($i,$jurusan);
        }
        return $fitness;
    }
	...
	public function simpan_jadwal(){
			$jadwal=$this->M_Penjadwalan2->get();
			foreach($jadwal->result() as $k);
				$semester_tipe=$k->semester_tipe;
				$tahun_akademik=$k->tahun_akademik;
				$jurusan=$k->kode_jur;
			$banyak_jurusan=$this->M_Penjadwalan2->cek_banyak_jurusan($semester_tipe,$tahun_akademik);
			$riwayat=$this->M_Penjadwalan2->semua_jadwal($semester_tipe,$tahun_akademik);
			foreach($banyak_jurusan as $b);
			if($b->banyak > 1 ){
				$cek=$this->M_Penjadwalan2->cek_semua_jadwal($semester_tipe,$tahun_akademik);
				foreach($cek as $j){
						$id=$j->kode_riwayat;
						
						$hapus=$this->M_Penjadwalan2->hapus_riwayat_jadwal($id);
					}
				...
				foreach($jadwal->result() as $j){
						...
						$simpan=$this->M_Penjadwalan2->simpan_jadwal($kode_pengampu,$kode_jam,$kode_hari,$kode_ruang);
					}
			}
			else{
				$cek=$this->M_Penjadwalan2->cek_jadwal($semester_tipe,$tahun_akademik,$jurusan);
				if($cek==true){
					foreach($jadwal->result() as $j){
						$kode_pengampu=$j->kode_pengampu;
						$kode_jam=$j->kode_jam;
						...
						$simpan=$this->M_Penjadwalan2->update_jadwal($kode_pengampu,$kode_jam,$kode_hari,$kode_ruang);
					}
				}
				else{
					foreach($jadwal->result() as $j){
