...
class M_Pengampu extends CI_Model{
	...
	function __construct(){
		...
		$sql  = "SELECT count(kode) as jumlah  from asisten";
	...
	function get($semester_tipe,$tahun_akademik){
		...
		$sql  = "SELECT a.kode as kode,".
				...
				"       b.nama as `nama_mk`,".
				"       b.jenis as `jenis_mk`,".
				"       c.kode as `kode_asisten`,".
				"       c.id_asisten,".
				"       c.nama as  `nama_asisten`,".
	...
	function delete_by_kode_asisten($kode_asisten){
        $this->db->query("DELETE FROM pengampu WHERE kode_asisten='$kode_asisten'");
    }
	...
	function cek_for_insert($kode_mk,$kode_asisten,$kelas,$tahun_akademik,$kode){		
		$rs = $this->db->query(
							   "SELECT CAST(COUNT(*) AS CHAR(1)) as cnt".
                               "FROM pengampu ".
							   "WHERE kode_mk='$kode_mk' AND ".
                               "      kode_asisten=$kode_asisten AND ".
                               "      kelas = '$kelas' AND ".
