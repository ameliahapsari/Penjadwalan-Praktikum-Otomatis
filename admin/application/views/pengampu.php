          ...
          <div class="modal-dialog-tambah">
            <div class="modal-content-tambah">
              ...
              <form id="simpan_pengampu">
				...
				<div class="form-group">
                  <label>Asisten</label>
                  <select name="asisten" class="form-control" role="menu">
				  <?php 
				  $asisten=$this->M_Asisten->semua_asisten();
			...
			$.ajax({
				...
				success: function(data){
					...
					if(data[i].kode_ruang == 0){
						...
						'<div class="modal-header">'+
							  ...
							  '<div class="modal-body">'+
								...
								'<div class="form-group">'+
								  '<label>Asisten</label>'+
								  '<select id="asisten_edit" name="asisten" role="menu" class="form-control"  >'+
										'<option value="'+data[i].kode_asisten+'">'+data[i].nama_asisten+'</option>'+
								  '</select>'+
								'</div>'+
								'<div class="form-group">'+
								  '<label>Semester</label>'+
					...
					$.ajax({
							type: 'ajax',
							method: 'get',
							url: '<?php echo base_url('index.php/asisten/semua_asisten'); ?>',
							async: false,
							dataType: 'json',
						   success: function(data){
								var html = '';
					...
					for(i=0; i<data.length; i++){
						...
						'<div class="modal-header">'+
								  '<button type="button" class="close" data-dismiss="modal" aria-label="Close">'+
									'<span aria-hidden="true">&times;</span>'+
								  '</button>'+
								'<h4 class="modal-title">Delete Pengajar '+data[i].nama_mk+' '+data[i].nama_asisten+' ?</h4>'+
							  '</div>'+
